...
bool DelaunayCore::DelaunayRouter::getRoute( const vector<int>& srcList, const vector<int>& tarList, vector<Route>& routeList )
{
	if (srcList.size() != tarList.size() || srcList.size() <= 0)
	{
		return false;
	}

	int nRoute = srcList.size();

	// fill half edge data
	struct RouteEdgeData
	{
		VtxPoint m_originalPoint;
	...
	const float offsetRatio = 0.5;
	for (DelaunayMesh::HalfedgeIter pH = m_mesh.halfedges_begin(); pH != m_mesh.halfedges_end(); ++pH)
	{
		Point p0 = m_mesh.point(m_mesh.from_vertex_handle(pH.handle()));
		Point p1 = m_mesh.point(m_mesh.to_vertex_handle(pH.handle()));
		Point p  = p0 * offsetRatio + p1 * (1-offsetRatio);
		
		int idx = m_mesh.edge_handle( pH.handle()).idx();
		routeEdgeData[idx].m_boundPoint = routeEdgeData[idx].m_originalPoint = VtxPoint(p[0], p[1]);
	}

	struct RouteData
	{
		VtxPoint m_srcPnt, m_tarPnt;
		VtxPoint m_srcNorPnt, m_tarNorPnt;
		vector<int> m_handleIDs;
	};
	vector<RouteData> routes(nRoute);
	for (int ithRoute = 0; ithRoute < nRoute; ++ithRoute)
	{
		int src = srcList[ithRoute];
		int tar = tarList[ithRoute];

		if (src < 0 || tar < 0 || src >= m_contourList.size() || tar >= m_contourList.size())
		{
			continue;
		}
		...
		{
			faceRoute.clear();
			findFaceRouteBFS(srcSeedHandles,
				tarSeedHandles,
				faceRoute,
				false,
				&srcVtxID, &tarVtxID);
		...
		// record route data
		RouteData& rData = routes[ithRoute];
		rData.m_srcPnt = VtxPoint(srcPnt.data()[0], srcPnt.data()[1]);
		rData.m_tarPnt = VtxPoint(tarPnt.data()[0], tarPnt.data()[1]);
		rData.m_srcNorPnt = rData.m_srcPnt + srcEntry.m_normal * (srcCtr.m_radius * m_endPointNormalRatio);
		rData.m_tarNorPnt = rData.m_tarPnt + tarEntry.m_normal * (tarCtr.m_radius * m_endPointNormalRatio);

		// transform face route into halfedge route
		int lastEdgeID = -1;
		for (int ithFace = 0; ithFace < faceRoute.size()-1; ithFace += m_routeStepSize)
		{
			FaceHandle thisFaceHanle = faceRoute[ithFace];
			for (FaceHalfedgeIter pNearH = m_mesh.fh_begin(thisFaceHanle); pNearH != m_mesh.fh_end(thisFaceHanle); ++pNearH)
			{
				...
				if (m_mesh.face_handle(oppoEdgeHandle) == faceRoute[ithFace+1])
				{
					EdgeHandle realEdgeHdl = m_mesh.edge_handle(edgeHandle);
					int curEdgeID = realEdgeHdl.idx();

					rData.m_handleIDs.push_back(curEdgeID);

					// fill adjacency data
					if (lastEdgeID != -1)
					{
						routeEdgeData[lastEdgeID].m_adjEdges.insert(curEdgeID);
						routeEdgeData[curEdgeID].m_adjEdges.insert(lastEdgeID);
					}
					if (ithFace == 0)
					{
						routeEdgeData[curEdgeID].m_adjEndPoints.push_back(rData.m_srcPnt);
					}
					else if (ithFace == faceRoute.size()-2)
					{
						routeEdgeData[curEdgeID].m_adjEndPoints.push_back(rData.m_tarPnt);
					}
	...
	for (int ithIter = 0; ithIter < 50; ++ithIter)
	{
		for (int ithHalfEdge = 0; ithHalfEdge < routeEdgeData.size(); ++ithHalfEdge)
		{
			...
			{
				int edgeID = *pN;
				avg = avg + routeEdgeData[edgeID].m_boundPoint;
				totWeight++;
			}
			...
			for (int i = 0; i < edgeData.m_adjEndPoints.size(); ++i)
			{
				...
				totWeight += endPointWeight;
			}
			avg = avg * (1/totWeight);
			edgeData.m_boundPoint = avg * smoothWeight + edgeData.m_boundPoint * (1-smoothWeight);
		}
	}

	// build edge routes
	routeList.resize(nRoute);
	for (int ithRoute = 0; ithRoute < nRoute; ++ithRoute)
	{
		int src = srcList[ithRoute];
		int tar = tarList[ithRoute];
		...
		Route& resultRoute = routeList[ithRoute];
		RouteData& data    = routes[ithRoute];
		resultRoute.reserve(data.m_handleIDs.size() + 2);
		resultRoute.push_back(data.m_srcPnt);
		//resultRoute.push_back(data.m_srcNorPnt);
