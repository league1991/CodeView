...
void CodeAtlas::FunctionUIItem::paint( QPainter *painter, const QStyleOptionGraphicsItem *option/* =0 */, QWidget *widget /* = 0 */ )
{
	...
	painter->setBrush(QBrush(QColor(239,228,176,255)));
	painter->setPen(QPen(QColor(150,150,150,255), 2));
	painter->drawEllipse(m_entityRect);
}

void CodeAtlas::FunctionUIItem::initIconList()
{
	if (m_publicImgArray[0].isEmpty())
		m_publicImgArray[0].loadImage(QImage("Resources/publicFunc.png"), QSize(150,150));
	if (m_publicImgArray[1].isEmpty())
		m_publicImgArray[1].loadImage(QImage("Resources/publicFuncHover.png"), QSize(150,150));
	if (m_publicImgArray[2].isEmpty())
		m_publicImgArray[2].loadImage(QImage("Resources/publicFuncSelected.png"), QSize(150,150));

	if (m_protectedImgArray[0].isEmpty())
		m_protectedImgArray[0].loadImage(QImage("Resources/protectedFunc.png"), QSize(150,150));
	if (m_protectedImgArray[1].isEmpty())
		m_protectedImgArray[1].loadImage(QImage("Resources/protectedFuncHover.png"), QSize(150,150));
	if (m_protectedImgArray[2].isEmpty())
		m_protectedImgArray[2].loadImage(QImage("Resources/protectedFuncSelected.png"), QSize(150,150));

	if (m_privateImgArray[0].isEmpty())
		m_privateImgArray[0].loadImage(QImage("Resources/privateFunc.png"), QSize(150,150));
	if (m_privateImgArray[1].isEmpty())
		m_privateImgArray[1].loadImage(QImage("Resources/privateFuncHover.png"), QSize(150,150));
	if (m_privateImgArray[2].isEmpty())
		m_privateImgArray[2].loadImage(QImage("Resources/privateFuncSelected.png"), QSize(150,150));
}