...
namespace CodeAtlas
{
	...
	{
	public:
		LodManager():m_lod(1.f),m_maxProjRadius(1), m_minProjRadius(1), m_avgProjRadius(1), m_maxClassRadius(1), m_symFilter(SymbolInfo::All){}
		~LodManager(){}
		...
		void setSymFilter(unsigned filter){m_symFilter = filter;}
		unsigned getSymFilter()const{return m_symFilter;}
	protected:
