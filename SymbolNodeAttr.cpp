...
void SymbolWordAttr::setWords( const SymbolNode::Ptr& node )
{
	...
	QStringList nameList = WordExtractor::normalizeWords(wordStr);
	for (int ithWord = 0; ithWord < nameList.size(); ++ithWord)
	{
		const QString& word = nameList[ithWord];
		int idx = addOrGetWord(word);
		QMap<int,float>::Iterator pIdx = m_nameWordWeightMap.find(idx);
		if (pIdx != m_nameWordWeightMap.end())
			pIdx.value()++;
		else
			m_nameWordWeightMap[idx] = 1;
	}
...
SymbolNodeAttr& SymbolWordAttr::copy( const SymbolNodeAttr& s )
{
	if (SymbolWordAttr* pS = castTo<SymbolWordAttr>(s))
	{
		...
		m_nameWordWeightMap = pS->m_nameWordWeightMap;
	}
	return *this;
}
...
{
	QString res;
	res += QString("pos: %1, %2   radius: %3  dataPtr:%4\n").arg(m_position.x()).arg(m_position.y()).arg(m_radius).arg((int)m_uiItem.data());
	return res;
}
